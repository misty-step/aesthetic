...
const focus = params.get('focus') === 'components' ? 'components' : '';
...
const spec = {
  ...
  button: { ...fallback.button, ...raw.button },
...
const vars = {
  ...
  '--button-height': spec.button?.height || spec.tokens.control,
  '--button-padding': spec.button?.padding || '0 1rem',
  '--button-weight': spec.button?.weight || '650',
};
...
const components = [
  ...
  specimen(
    'Badges',
    '<div class="row wrap"><span class="badge accent">' +
      icon('activity') +
      'Running</span><span class="badge success">' +
      icon('check') +
      'Healthy</span><span class="badge warning">' +
      icon('alert') +
      'Needs review</span><span class="badge danger">' +
      icon('close') +
      'Blocked</span><span class="badge neutral">Draft</span></div>',
  ),
  specimen(
    'Alert',
  ...
  specimen(
    'Table',
    '<div class="table-wrap"><table><thead><tr><th>Signal</th><th>Status</th><th>Age</th></tr></thead><tbody><tr><td>API latency</td><td><span class="status-glyph success">' +
      icon('check') +
      '</span>Healthy</td><td>2m</td></tr><tr><td>Delivery queue</td><td><span class="status-glyph warning">' +
      icon('alert') +
      '</span>Review</td><td>18m</td></tr><tr><td>Auth probe</td><td><span class="status-glyph danger">' +
      icon('close') +
      '</span>Failed</td><td>1h</td></tr></tbody></table></div>',
    'wide',
  ),
  specimen(
    'Dialog',
...
const html = `<main class="future-root lab-stage lab-spec layout-${spec.layout} components-${spec.componentLayout} mode-${mode}${focus ? ` focus-${focus}` : ''}" data-id="${spec.id}" style="${Object.entries(
  vars,
)
  .map(([k, v]) => `${k}:${v}`)
  .join(';')}">
  ...
  <div class="future-scroll"><div class="future-content" id="top">
    ...
    <section id="buttons" class="section future-section button-lab"><header class="section-title section-heading section-head"><span>01</span><div><h2>Action controls</h2><p>The primary craft problem in this round: square, unmistakable, complete.</p></div></header><div class="button-board"><article><small>Family grammar</small><h3>${esc(spec.button?.grammar)}</h3><dl>${['primary', 'secondary', 'ghost', 'destructive', 'focus', 'pressed', 'loading'].map((key) => `<div><dt>${key}</dt><dd>${esc(spec.button?.[key])}</dd></div>`).join('')}</dl></article><article><small>Core family</small><div class="button-stage"><button class="btn primary">Create report</button><button class="btn secondary">Preview</button><button class="btn ghost">Dismiss</button><button class="btn danger">Remove</button><button class="btn icon-btn" aria-label="More">${icon('dots')}</button></div></article><article><small>Interaction matrix</small><div class="button-matrix"><button class="btn primary is-hover">Hover</button><button class="btn secondary is-focus">Focus</button><button class="btn secondary is-pressed">Pressed</button><button class="btn secondary is-loading" data-loading="${esc(spec.button?.loading)}"><span class="loading-mark"><i></i><i></i><i></i></span>Loading</button><button class="btn secondary" disabled>Disabled</button><button class="text-link">Text link →</button></div></article></div></section>
    <section id="foundations" class="section future-section foundations"><header class="section-title section-heading section-head"><span>02</span><div><h2>Foundations</h2><p>The full token grammar, not a moodboard.</p></div></header>
    ...
    <section id="compositions" class="section"><header class="section-title"><span>05</span><div><h2>Composition stress test</h2><p>The same system across four neutral product archetypes.</p></div></header><div class="composition-grid">
      ...
      <article class="composition workbench"><header><span>Evidence queue</span><span class="badge accent">24 open</span></header><div class="work-rows">${[
        ...
        .map(
          ([a, b, c], i) =>
            `<div><span><i class="status-glyph ${i === 0 ? 'danger' : i === 1 ? 'warning' : i === 2 ? 'success' : 'accent'}">${icon(i === 0 ? 'close' : i === 1 ? 'alert' : i === 2 ? 'check' : 'activity')}</i>${a}</span><b>${b}</b><code>${c}</code></div>`,
        )
        .join('')}</div></article>
    </div></section>
...
window.__LAB_READY__ = {
  ...
  sections: 7,
  buttonStates: 10,
  displayHeader: getComputedStyle(document.querySelector('.type-display'))
    .fontSize,
