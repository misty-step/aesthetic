...
let selected = localStorage.getItem('lab003:selected') || 'BASE-1',
  mode = localStorage.getItem('lab003:mode') || 'light';
...
function select(id) {
  ...
  localStorage.setItem('lab003:selected', selected);
  document
    .querySelectorAll('.option')
    .forEach((b) => b.classList.toggle('active', b.dataset.id === selected));
  const spec = specs.find((s) => s.id === selected);
  preview.src = `frame.html?id=${encodeURIComponent(selected)}&mode=${mode}&v=3`;
...
modeButton.onclick = () => {
  ...
  localStorage.setItem('lab003:mode', mode);
